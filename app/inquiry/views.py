...
class InquiryView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        home_country_options = set(AskTagHomeCountry.objects.all().values_list('body__name', flat=True))
        study_abroad_country_options = set(AskTagStudyAbroadCountry.objects.all().values_list('body__name', flat=True))
        school_options = set(AskTagSchool.objects.all().values_list('body__name', flat=True))
        major_options = set(AskTagMajor.objects.all().values_list('body__name', flat=True))
...
def get_search_result(query_params, user):
    ...
    for key in query_params.keys():
        ...
        if key == 'status':
            ...
            ask_id_list = AskTagStudyAbroadCountry.objects.filter(body__name__in=value_list).values_list('ask__id', flat=True)
        elif key == 'schools':
            ask_id_list = AskTagSchool.objects.filter(body__name__in=value_list).values_list('ask__id', flat=True)
        elif key == 'majors':
            ask_id_list = AskTagMajor.objects.filter(body__name__in=value_list).values_list('ask__id', flat=True)
        elif key == 'start_year':
